...
class grpcWSGI(grpc.Server):
    ...
    def _get_rpc_handler(self, environ):
        ...
        func_name = camel_to_snake(path.split("/")[-1])
        handler_call_details = _HandlerCallDetails(path, None)

        rpc_handler = None
        for handler in self._handlers:
            # print("handler", handler)
            call_handler = getattr(handler, func_name, None)
            rpc_handler = handler.__mapping__().get(path)
            print("rpc_handler", rpc_handler)
            print("call_handler", call_handler)
            if rpc_handler and call_handler:
                return (rpc_handler, call_handler)
    ...
    def _do_grpc_request(self, rpc_method, call_method, environ, start_response):
        request_data = self._read_request(environ)
        context = self._create_context(environ)

        # request_obj = rpc_method.request_type

        print("request_data", request_data)
        # print("request_data", request_obj)
        print("request_obj type", type(request_data))

        # request_obj = request_obj.parse(bytes(request_data))
        # print("request_data trial", request_obj.to_dict())
        print("request_data", request_data)
        # _, _, message = protocol.unwrap_message(request_data)

        # message = rpc_method.request_deserializer(request_data)

        request_proto = rpc_method.request_type().FromString(request_data)
        print("request_proto", request_proto.to_dict())

        resp = None

        try:
            if rpc_method.cardinality == grpclib.const.Cardinality.UNARY_UNARY:
                resp = call_method(request_proto)
            elif rpc_method.cardinality == grpclib.const.Cardinality.UNARY_STREAM:
                resp = rpc_method.func(request_proto, context)
        ...
        #     )
        if rpc_method.cardinality == grpclib.const.Cardinality.UNARY_STREAM or rpc_method.cardinality == grpclib.const.Cardinality.STREAM_STREAM:
            yield from self._do_streaming_response(
                rpc_method, start_response, context, headers, resp
            )
    ...
    def __call__(self, environ, start_response):
        ...
        rpc_method, call_method = self._get_rpc_handler(environ)
        request_method = environ["REQUEST_METHOD"]

        if rpc_method:
            if request_method == "POST":
                return self._do_grpc_request(
                    rpc_method, call_method, environ, start_response
                )
